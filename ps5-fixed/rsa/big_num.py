...
class BigNum(object):
  ...
  def slow_mul(self, other):
    ...
    for i in xrange(0, len(self.d)):
        ...
        for j in xrange(0, len(other.d)):
            if i == 0 and j == 0:
                word = self.d[i]*other.d[j]
                result.d[0] = word.lsb()
                carry = word.msb()
            else:
                multiply = self.d[i]*other.d[j]  
                
                carry_of_multiply = multiply.msb()
                lsb_of_miltiply = multiply.lsb()
                
                add = result.d[i + j ] + carry
                add_carry = add.msb()
                lsb_of_add = add.lsb()
                
                final_lsb = (lsb_of_add + lsb_of_miltiply).lsb()
                
                final_lsb_carry = (lsb_of_add + lsb_of_miltiply).msb()
                
                result.d[i + j ] = final_lsb
                final_carry_test = (carry_of_multiply + add_carry)
                
                final_carry = final_carry_test.lsb() + final_lsb_carry
                carry = final_carry.lsb()
  ...
  def slow_divmod(self, other):
    ...
    '''
    # if other equals one 
    if len(other.d) == 1 and other == BigNum.one():
        return (self, BigNum.zero())
    
    # if other is bigger than self, quotient should be zero and remainder should be denominator
    ...
    while s0 <= self:
       s.append(s0) 
       s0 = s0 + s0
    # while break , s is greater than self 
