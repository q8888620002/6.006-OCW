...
class PriorityQueue:
    ...
    def _min_heapify(self,i,heapindex):
        left = 2 * i + 1 
        right = 2 * i + 2 
        smallest = i 
        if left <= heapindex and self.queue[left] < self.queue[smallest]:
            smallest = left 
        if right <= heapindex and self.queue[right] < self.queue[smallest]:
            smallest = right
        if smallest != i:
            self.queue[i],self.queue[smallest] = self.queue[smallest],self.queue[i]
            self._min_heapify(smallest,heapindex)

    def heap_dec_key(self,index,key):
        while ((index -1 )// 2 ) > -1 and key < self.queue[((index -1 )// 2 )]:
            parent = (index-1)//2
            self.queue[index], self.queue[parent] = self.queue[parent], key
            index = parent 
    ...
    def pop(self):
        ...
        if len(self.queue) == 0:
            return None
        self._find_min()
        self.queue[self.min_index], self.queue[-1] = self.queue[-1], self.queue[self.min_index]
        popped_key = self.queue.pop(-1)
        self.min_index = None
        heapindex = self.__len__() - 1
        self._min_heapify(0,heapindex)
        
        return popped_key

    def build_min_heap(self):
        heapindex = self.__len__() - 1
        for i in xrange(self.__len__()//2, -1, -1):
            self._min_heapify(i,heapindex)
